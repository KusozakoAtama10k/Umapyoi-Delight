...
public class UDModMenuIntegration implements ModMenuApi {
    @Override
    public ConfigScreenFactory<?> getModConfigScreenFactory() {
        return parentScreen -> YetAnotherConfigLib.createBuilder()
                .title(configText("title"))
                .categories(buildCategories())
                .save(UDConfig.HANDLER::save)
                .build()
    ...
    private static Collection<ConfigCategory> buildCategories() {
        ...
        for (Field field : UDConfig.class.getFields()) {
...
//                option = createOption(field, controllerBuilder);
    ...
    private static <T> Option<T> createOption(
        ...
        var defaults = UDConfig.HANDLER.defaults();
        ...
        return Option.<T>createBuilder()
                ...
                .binding(defaultValue,
                        () -> {
                            try {
                                @SuppressWarnings("unchecked")
                                T value = (T) field.get(Umapyoidelight.CONFIG);
                                return value;
                            } catch (IllegalAccessException e) {
                                throw new RuntimeException(e);
                            }
                        ...
                        val -> {
                            try {
                                ...
                                field.set(Umapyoidelight.CONFIG, val);
                            } catch (IllegalAccessException e) {
                                throw new RuntimeException(e);
                            }
                        }
    ...
    private static Method getValueFormatterMethod(String methodName, Class<?> param) {
        ...
        if (!methodName.isEmpty()) {
            try {
                valueFormatterMethod = UDConfig.class.getMethod(methodName, param);
    ...
    private static Component configText(String key) {
        return Component.translatable("config.umapyoidelight." + key);
    }

    private static Component optionalConfigText(String key) {
        return Component.translatableWithFallback("config.umapyoidelight." + key, "");
    }
}